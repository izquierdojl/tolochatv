#!/bin/sh
set -e
# Entrypoint: fix permissions and drop to tolochatv user
#
...
if [ "$(stat -c '%U' /app/cache/users)" != "tolochatv" ]; then
    chown -R tolochatv:tolochatv /app/cache/users 2>/dev/null || true
fi
# Ensure writable even on filesystems that ignore chown (e.g., some NAS mounts)
if ! gosu tolochatv sh -c "touch /app/cache/users/.perm_test && rm /app/cache/users/.perm_test" 2>/dev/null; then
    chmod -R u+rwX,g+rwX /app/cache/users 2>/dev/null || true
    chmod g+s /app/cache/users 2>/dev/null || true
fi
# Final verification - warn if still not writable
if ! gosu tolochatv sh -c "touch /app/cache/users/.perm_test && rm /app/cache/users/.perm_test" 2>/dev/null; then
    echo "WARNING: /app/cache/users is not writable by tolochatv user"
    echo "Cache operations may fail. Check volume permissions."
fi
